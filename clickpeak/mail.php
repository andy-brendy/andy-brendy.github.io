...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  if (!empty($_POST['uname']) && !empty($_POST['form-name']) && !empty($_POST['uemail']) && !empty($_POST['uphone'])){
    ...
    if (isset($_POST['usite'])) {
      if (!empty($_POST['usite'])){
        $usite = strip_tags($_POST['usite']);
        $usiteFieldset = "<b>Сайт:</b>";
      }
    }

    $to = "info@clickpeak.ru"; /*Укажите адрес, на который должно приходить письмо*/
    $sendfrom = "mail@info@clickpeak.ru/"; /*Укажите адрес, с которого будет приходить письмо */
  ...
  header ("Location: https://clickpeak.ru/"); // главная страница вашего лендинга
}
?>